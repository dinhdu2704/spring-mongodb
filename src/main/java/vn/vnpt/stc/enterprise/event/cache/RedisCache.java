...
import vn.vnpt.stc.enterpise.commons.utils.ObjectUtils;
import vn.vnpt.stc.enterprise.event.serialize.GenericSerializer;
...
public class RedisCache implements ObjectCache {
    ...
    @Override
    public void
    put(String key, Object item, int ttl, Class<?> type) {
        try {
            String value = serializer.serialize(item);
            valueOperations.set(cacheKey(type,key),value, ttl, TimeUnit.SECONDS);
        } catch (Exception e) {
            logger.error("Cannot serialize object of type {}",type.getSimpleName(),e);
        }
    }
